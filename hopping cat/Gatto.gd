...
func _physics_process(delta):
	...
	if Input.is_action_pressed("indietro"):
		display("v")
		move_vec.z -= 1
		
	if Input.is_action_pressed("avanti"):
		display("^")
		move_vec.z += 1
	if Input.is_action_pressed("ruotad"):
		display("->")
		ruota_camera(1,0)
		#move_vec.x += 1
	if Input.is_action_pressed("ruotas"):
		display("<-")
		ruota_camera(-1,0)
		#move_vec.x -= 1
	if Input.is_action_pressed("alza"):
		display("/")
		ruota_camera(0,1)
		#move_vec.x += 1
	if Input.is_action_pressed("abbassa"):
		display("\\")
		ruota_camera(0,-1)
	...
	if grounded and Input.is_action_just_pressed("salta"):
		display("^^^^")
		just_jumped = true
		y_velo = JUMP_FORCE
